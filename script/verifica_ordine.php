...
echo "<h1>🔍 Verifica Ordine #$orderId ($prefix)</h1>";
...
echo "<h3>✅ Record in Database Locale (paypal):</h3>";
...
while($row = $resMdl->fetch_assoc()) {
    echo "<tr>
            ...
            <td>{$row['courseid']}</td>
            <td align='right'><b>" . number_format($row['cost'], 2) . " €</b></td>
          </tr>";
}
echo "</table>";

// 3. DETTAGLIO COMPOSIZIONE (Esplosione da WooCommerce)
echo "<h3>📦 Dettaglio Composizione (Da WooCommerce Sorgente):</h3>";
...
while($item = $resWc->fetch_assoc()) {
    $q = $item['qta'] ?: 1;
    echo "• <b>{$item['order_item_name']}</b>: Quantità x $q (Totale riga: {$item['totale']} €)<br>";
}
echo "</div>";

echo "<br><a href='index.php'>⬅️ Torna al Cron</a></body>";